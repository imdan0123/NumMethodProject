...
plot(tspan,L_ave,'-m','LineWidth',2); 
plot(tspan,I_ave,'--k','LineWidth',2);
plot(tspan,R_ave,'-c','LineWidth',2);
plot(tspan,P_ave,'--r','LineWidth',2);
plot(tspan,E_ave,'--g','LineWidth',2);
plot(tspan,F_ave,'-b','LineWidth',2);
plot(tspan,B_ave,'--m','LineWidth',2);
legend('Susceptible','Latent','Infected','removed',' Total Population','external','spores','berry P',{'Susceptible'},'Location','NorthWest');
...
function dydt = Pb_model(P, T, F, A)
    % Implement Pb model equation
    % Example (replace with your actual equation):
    dydt = (0.1724 * P^9 - 0.0000212 * P^2) * T * E;
end

function dydt = Pl_model(P, T, E, mu_L, dt, mu_I, I)
    % Implement Pl model equation
    % Example (replace with your actual equation):
...
% 
% 
% to add plotting of other elements and optional
%things like movies