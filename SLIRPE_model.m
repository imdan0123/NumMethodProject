...
function [dydt] = SLIRPE_model(idx,y,e,mu_L,p)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    dydt(1) = Pb_model(vine(cnt).P(t-1), T_used, F, A);
    dydt(2) = dydt(1) + Pl_model(vine(cnt).P(t-1), T_used, E, mu_L_used, dt, mu_I, I);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if I > 0
    dydt(8) = Gamma * exp(alpha * I * A) - F * R_frac; % Implement F function
else
    dydt(8) = 0;
end
