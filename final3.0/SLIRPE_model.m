...
if I > 0
    ...
    if(I==0)%spore production shouldn't start before infection (quirk of exponential curve fit)
        ...
        A_norm = A / 1e4;
        % production rate
        sporeProduction = Gamma * exp(alpha * I * A_norm);
        load EnvironmentalForcing.mat

        % parameters
        Mkappa = kappa * sqrt(U(idx)^2+V(idx)^2);
        sporeEscape = sporeProduction * exp(Mkappa + xi) / (eta * (exp(Mkappa + xi) + 1));
        effectiveReleaseRate = sporeEscape;
        % the rate of change for spores 
        dydt(8) = sporeProduction - effectiveReleaseRate;
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end
